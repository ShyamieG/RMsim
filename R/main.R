...
#' @usage run.RM(N_h,N_h_t0, N_v, N_v_t0, runtime, bite_rate, raw_hv_trans_rate, raw_vh_trans_rate, eff_hv_trans_rate, eff_vh_trans_rate, h_rec_rate, v_rec_rate, h_lag, v_lag, h_max_duration, v_max_duration, mean_hyp = 0, hyp_act_rate, hyp_death_rate, prev_sim_output = NULL, XXX, verbose = TRUE)
#' @details
#' `run.RM()` is the main function of the RMsim package. It simulates an infection scenario under an agent-based Ross-Macdonald (RM) model given a set of user-specified parameters.
#' The `prev_sim_output` argument allows for the simulation parameters to vary temporally (see 'Examples').
#' The `return_proportion_infected` argument allows users to drop the object that stores the proportion of infected hosts and vectors at every time step. The must be TRUE to use the `plot.inf.over.time` and `plot.inf.histogram` functions on the resulting object. However, for very long simulations, this object can become extremely large. Therefore, if it is determined that this object is not needed, it might be advisable to set this argument to FALSE to conserve disk space and memory. If TRUE, the frequency with which the proportion of infected individuals is calculated and recorded can be set using `proportion_infected_interval`.
#' The `prune_regularly` and `pruning_interval` arguments allow pruning to be done dynamically, while the simulation is running. Similarly, if `sampling_df` is specified, sampling will be done dynamically as well. This flexibility can be useful when running very long simulations.
#' `sample_df` should be a data frame with 1 row per sampling even and 5 or 6 columns having the following names: 'time_step', 'population', 'proportion' and/or 'number', 'sample_post_lag', and 'resample_possible'. See [sample.RM()] for details on what these columns should contain. RMsim::sampling_df gives an example of the expected object.
...
#' @param prev_sim_output output of a previous simulation to continue from. See 'Details'. Default is NULL.
#' @param sampling_df a data frame specifying how to sample infections in the simulation. See 'Details'. Default is NULL.
#' @param return_proportion_infected should the `proportion_infected` object be returned in the function output. See 'Details'. Default is TRUE
#' @param proportion_infected_interval if `return_proportion_infected` is TRUE, how often should the proportion of infected individuals be calculated? See 'Details'. Default is 1 (i.e. every day).
#' @param prune_regularly should the infection record be pruned at regular intervals (set with the `pruning_interval` argument) to remove events that do not contribute to the history of any samples or active infections? This additional step can save hard disk space and memory when running very long simulations but may not be necessary for shorter simulations. See [prune.infection.record()] for more details. Default is FALSE.
#' @param pruning_interval if `prune_regularly` is TRUE, how frequently should this be done? Default is every 10000 days, but this may not be optimal for all cases.
#' @param verbose should messages about simplifying steps be enabled? Default is TRUE.
#' @returns A named list containing the user-specified input parameters (`input_parameters`); the calculated Ross-Macdonald parameters (`RM_parameters`); an infection record (`infection_record`); a vector that summarizes the infection status of each individual (`inf_age`); if requested with `return_proportion_infected`, a data frame that records the proportion of infected hosts and vectors at each simulated time step (`proportion_infected`).\cr\cr
...
run.RM <- function(N_h,
                   ...
                   return_proportion_infected=TRUE,
                   proportion_infected_interval=1,
                   sampling_df=NULL,
                   prune_regularly=FALSE,
  ...
  if (is.null(prev_sim_output)) {
    if (is.null(N_h_t0) & is.null(N_v_t0)) {
      stop("'N_h_t0' and 'N_v_t0' cannot both be undefined - the simulation must be initiated by infected individuals.")
    }
    if (is.null(N_h_t0)) {N_h_t0 <- 0}
    if (is.null(N_v_t0)) {N_v_t0 <- 0}
  ...
  if (!is.logical(return_proportion_infected)) {
    stop("'return_proportion_infected' must be TRUE or FALSE.")
  }
  if (!is.logical(prune_regularly)) {
    stop("'prune_regularly' must be TRUE or FALSE.")
  }
  if (!is.logical(verbose)) {
    stop("'verbose' must be TRUE or FALSE.")
  }

  # If prune_regularly, ensure that pruning interval is valid
  if (prune_regularly) {
    if (!is.numeric(pruning_interval) | pruning_interval < 0 | pruning_interval %% 1 != 0) {
      stop("'pruning_interval' must be a positive integer.")
    }
  }
  ...
  if (return_proportion_infected) {
    if (!is.numeric(proportion_infected_interval) | proportion_infected_interval < 0 | proportion_infected_interval %% 1 != 0) {
      stop("'proportion_infected_interval' must be a positive integer.")
    }
  }

  # Check format of sampling_df object, if provided
  if (!is.null(sampling_df)) {
    sampling_df <- data.table::as.data.table(sampling_df)
    if ('time_step' %nin% colnames(sampling_df) | 'population' %nin% colnames(sampling_df) | ('proportion' %nin% colnames(sampling_df) & 'number' %nin% colnames(sampling_df)) | 'sample_post_lag' %nin% colnames(sampling_df) | 'resample_possible' %nin% colnames(sampling_df)) {
      stop("At least 1 required column is missing from 'sampling_df'.")
    }
  }
  ...
  if (is.null(prev_sim_output)) {
    ...
    current_inf <- max(infection_record$inf_id) + 1
  ...
  if (mean_hyp > 0) {
    if (is.null(prev_sim_output)) {
      ...
      if (prev_sim_output$input_parameters["N_h", ncol(prev_sim_output$input_parameters)] == N_h) {
        ...
        n_hypno <- unlist(lapply(prev_sim_output$hyp_reservoir, length))
        names(n_hypno) <- names(hyp_reservoir)
      } else {
        old_hIDs <- names(prev_sim_output$inf_age)[grep("H", names(prev_sim_output$inf_age))]
        max_N_h = max(c(N_h, as.double(prev_sim_output$input_parameters["N_h",])))
        hyp_reservoir <- list();length(hyp_reservoir) <- N_h;names(hyp_reservoir) <- hIDs
        ...
        n_hypno[old_hIDs, 1:ncol(n_hypno)] <- unlist(lapply(prev_sim_output$hyp_reservoir, length))
      }
    }
  }
  ...
  if (return_proportion_infected) {
    if (proportion_infected_interval != 1) {
      time.steps0 <- time.steps[which(time.steps %% proportion_infected_interval == 0)]
    }
    if (is.null(prev_sim_output)) {
      proportion_infected <- as.data.frame(matrix(nrow=2, ncol=length(time.steps0)))
      rownames(proportion_infected) <- c("H", "V")
      colnames(proportion_infected) <- time.steps0
    } else {
      proportion_infected <- prev_sim_output$proportion_infected
      proportion_infected[, as.character(time.steps0)] <- NA
    }
  }
  ...
  for (x in time.steps) {
    if (x == 1) {
      ...
      if (return_proportion_infected) {
        if (x %in% time.steps0) {
          proportion_infected[,as.character(x)] <- 0
        }
      }
      ...
      for (type in c("H", "V")) {
        if (length(grep(type, new_inf_indivs)) > 0) {
          inf_ids <- current_inf:(current_inf+length(grep(type, new_inf_indivs))-1)
          current_inf <- current_inf+length(grep(type, new_inf_indivs))
          ...
          infection_record[current_row:(current_row + length(inf_ids) - 1), names(infection_record) := new_infections]
          current_row <- current_row + length(inf_ids)
        }
      };rm(type, inf_ids)
      ...
      if (return_proportion_infected) {
        if (x %% proportion_infected_interval == 0) {
          for (type in c("h", "v")) {
            n_infected <- sum(!is.na(inf_age[get(paste0(type, "IDs"))]))
            proportion_infected[toupper(type), as.character(x)] <- n_infected/get(paste0("N_", type))
          };rm(type, n_infected)
        }
      }
      ...
      if (nrow(biting_events) > 0) {
        ...
        if (nrow(transmission_events) > 0) {
          ...
          # add these events to the infection record
          inf_ids <- current_inf:(current_inf+length(infecteds)-1)
          current_inf <- current_inf+length(infecteds)
      ...
      if (mean_hyp > 0) {
        if (sum(n_hypno) > 0) {
          ...
          if (nrow(activation_events) > 0) {
            inf_ids <- current_inf:(current_inf + nrow(activation_events)-1)
            current_inf <- current_inf+length(grep(type, new_inf_indivs))
      ...
      # -- Step 4 - Check simulation status and simplify infection record by pruning, if specified
      ...
      if (prune_regularly & x %% pruning_interval == 0) {
        if (verbose == T) {
          message(paste("pruning record on day", x))
        }
        # store empty rows to add back later
        empty_rows <- infection_record[is.na(inf_id),]
        # only keep active infections, or sample infections
        focal_infections <- unique(c(infection_record[is.na(infection_record$end_t) & !is.na(infection_record$inf_id)]$inf_id, infection_record[infected=="sample"]$inf_id))
        infection_record <- prune.infection.record(infection_record, focal_infections=focal_infections)
        current_row = nrow(infection_record) + 1
        current_inf = max(infection_record$inf_id) + 1
        infection_record <- data.table::rbindlist(list(infection_record, empty_rows), fill = TRUE)
        total_rows <- nrow(infection_record)
      ...
      # -- Step 5 - Sample active infections, if specified
      if (!is.null(sampling_df)) {
        if (x %in% sampling_df$time_step) {
          if (verbose == T) {
            message(paste("sampling infections on day", x))
          }
          # store empty rows to add back later
          empty_rows <- infection_record[is.na(inf_id),]
          sampling_dat <- sampling_df[time_step==x,]
          if (sampling_dat$proportion=="NULL") {sampling_dat_proportion=NULL} else {sampling_dat_proportion=sampling_dat$proportion}
          if (sampling_dat$number=="NULL") {sampling_dat_number=NULL} else {sampling_dat_number=sampling_dat$number}
          sampled <- sample.RM(RM_out=list(input_parameters=input_parameters, infection_record=infection_record[!is.na(inf_id),]),
                               time_step = x,
                               population = sampling_dat$population,
                               proportion = sampling_dat_proportion,
                               number = sampling_dat_number,
                               sample_post_lag = sampling_dat$sample_post_lag,
                               resample_possible = sampling_dat$resample_possible,
                               sort_events = FALSE)
          infection_record <- sampled$infection_record
          current_row = nrow(infection_record) + 1
          current_inf = max(infection_record$inf_id) + 1
          infection_record <- data.table::rbindlist(list(infection_record, empty_rows), fill = TRUE)
          total_rows <- nrow(infection_record)
    ...
    if (x == max(time.steps)) {
      ...
      output <- list(input_parameters=input_parameters, RM_parameters=RM_parameters, infection_record=infection_record, inf_age=inf_age)
      if (return_proportion_infected) {
        output <- append(output, list(proportion_infected=proportion_infected))
      }
      if (mean_hyp > 0) {
        output <- append(output, list(hyp_reservoir=hyp_reservoir))
      }
      return(output)
    }
  }
