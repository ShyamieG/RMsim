...
#' @details
#' Because RMsim allows for a lag between infection and contagiousness, as well as an upper limit on the duration of infection, we distinguish between 'raw' transmission rates and 'effective' transmission rates. This function converts between these two types of transmission rate.\cr\cr
...
#' The 'effective' transmission rate represents the overall probability that an infection will be transmitted given that transmission cannot occur during the `lag` period and after the `max_duration` period. The 'effective' transmission rate is used to calculate the expected Ross-Macdonald parameters, including the expected proportion of infected hosts and vectors at equilibrium.\cr\cr
#' Either 'raw_trans_rate' or 'eff_trans_rate' must be specified, but both cannot be specified at the same time.
...
#' @param recovery_rate per day rate of individual recovery from infection
...
#' @param eff_trans_rate overall probability that an infection will be transmitted (used for Ross-Macdonald parameter calculations)
...
#' @details
#' This function implements the fundamental Ross-Macdonald equations to calculate the proportion of the host and vector populations that are infected at equilibrium. It is used by the main `run.RM()` function as well as plotting functions `plot.inf.over.time()` and `plot.inf.histogram()`. It is also useful for quickly checking if a given set of epidemiological parameters is plausible/possible.
